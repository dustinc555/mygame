...
func _add_animated_avatar(pack_key: String, source_player: AnimationPlayer, animation_spec: Dictionary, position: Vector3) -> void:
	...
	var source_skeleton := _find_skeleton(source_player.get_parent())
	var position_scale := AnimationPositionScale.ratio_between(source_skeleton, target_skeleton)
	for animation_name in animation_names:
		var source_animation := source_player.get_animation(animation_name)
		var retargeted_animation := _retarget_animation(pack_key, source_animation, avatar, target_skeleton, should_loop_animation, position_scale)
...
func _retarget_animation(_pack_key: String, source_animation: Animation, target_root: Node3D, target_skeleton: Skeleton3D, loop_animation: bool = true, position_scale: float = 1.0) -> Animation:
	return AnimationRetargetLib.retarget_animation(source_animation, target_root, target_skeleton, loop_animation, position_scale)
...
func _find_animation_player(root: Node) -> AnimationPlayer:
	return AnimationRetargetLib.find_animation_player(root)


func _find_skeleton(root: Node) -> Skeleton3D:
	return AnimationRetargetLib.find_skeleton(root)
