extends Node

class_name PerceptionController

const WORLD_ACTOR_RULES := preload("res://scripts/world_sim/world_actor_rules.gd")

const SNEAK_DETECTION_PERCEPTION_XP_PER_SECOND := 0.08
const SNEAKING_RISK_XP_PER_SECOND := 5.0
const SNEAKING_RISK_SCORE_FLOOR := 0.02
const SNEAKING_RISK_PRESSURE_CAP := 1.0
const SNEAKING_IDLE_XP_MULTIPLIER := 0.5
const SNEAKING_MOVING_SPEED_THRESHOLD := 0.18
const SNEAKING_OBSERVER_GRACE_LEVELS := 4.0
const SNEAKING_OBSERVER_FADE_LEVELS := 12.0
const SNEAKING_NOVICE_MIN_TRAINING_PRESSURE := 0.45
const SNEAKING_NOVICE_PRESSURE_FLOOR_END_LEVEL := 10.0
const SUSTAINED_MOVING_EXPOSURE_MIN_LIGHT := 0.25
const SUSTAINED_MOVING_EXPOSURE_SCORE_CAP := 0.22
const SUSTAINED_MOVING_EXPOSURE_IDLE_DECAY_PER_SECOND := 0.75
const MOVEMENT_MODE_SNEAK := 2
const DEFAULT_BODY_HEIGHT := 2.0
const DEFAULT_BODY_RADIUS := 0.45

@export var observer_radius := 18.0
@export var view_distance := 15.0
@export var view_cone_degrees := 105.0
@export var clear_seen_threshold := 0.36
@export var partial_seen_threshold := 0.14
@export var standing_posture_visibility := 1.0
@export var novice_sneak_posture_visibility := 0.98
@export var master_sneak_posture_visibility := 0.18
@export var sneak_posture_skill_curve := 0.75
@export var sneak_contested_skill_curve := 28.0
@export var novice_close_reveal_multiplier := 1.12
@export var master_close_reveal_multiplier := 0.12
@export var close_reveal_skill_curve := 0.8
@export var novice_clear_los_visibility_floor := 0.35
@export var novice_suspicion_seconds := 0.55
@export var master_suspicion_seconds := 4.0
@export var perception_tick_seconds := 0.12
@export_range(1, 128, 1) var max_observer_candidate_checks_per_subject := 64
@export_range(1, 64, 1) var max_observers_per_subject := 16
@export_range(1, 128, 1) var max_cached_stealth_lights := 32
@export_range(1, 32, 1) var max_stealth_lights_per_subject := 8

var root_scene: Node
var hud_layer: CanvasLayer
var debug_show_los_rays := false

var _gecs_world_cache: Node
var _world_time_controller_cache: Node
var _day_night_lighting_controller_cache: Node
var _latest_results_by_subject_id: Dictionary = {}
var _latest_results_by_pair: Dictionary = {}
var _indicators: Dictionary = {}
var _suspicion_states: Dictionary = {}
var _debug_ray_mesh_instance: MeshInstance3D
var _debug_ray_mesh := ImmediateMesh.new()
var _debug_ray_material := StandardMaterial3D.new()
var _initialized := false
var _perception_tick_accumulator := 0.0
var _active_subject_scan_remaining := 0.0
var _local_light_cache: Array[Light3D] = []
var _local_light_cache_initialized := false
var _last_sim_state: Dictionary = {"active_subject_count": 0, "observer_count": 0, "pair_count": 0}


func initialize(target_root: Node, target_hud: CanvasLayer = null) -> void:
	root_scene = target_root
	hud_layer = target_hud
	_try_initialize()


func _ready() -> void:
	add_to_group("perception_controller")
	_try_initialize()


func update_sim(delta: float) -> void:
	if not _try_initialize():
		return
	if _latest_results_by_subject_id.is_empty() and _latest_results_by_pair.is_empty():
		_active_subject_scan_remaining = maxf(0.0, _active_subject_scan_remaining - delta)
		if _active_subject_scan_remaining > 0.0:
			return
	var active_subject_ids := _get_active_sneaking_subject_ids()
	if active_subject_ids.is_empty():
		_perception_tick_accumulator = maxf(perception_tick_seconds, 0.0)
		_active_subject_scan_remaining = maxf(perception_tick_seconds, 0.0)
		_clear_perception_state()
		_last_sim_state = {"active_subject_count": 0, "observer_count": 0, "pair_count": 0}
		return
	_active_subject_scan_remaining = 0.0
	_perception_tick_accumulator += delta
	var should_update := _latest_results_by_subject_id.is_empty()
	for subject_id in active_subject_ids:
		if not _latest_results_by_subject_id.has(subject_id):
			should_update = true
			break
	var tick_seconds := maxf(perception_tick_seconds, 0.0)
	if not should_update and tick_seconds > 0.0 and _perception_tick_accumulator < tick_seconds:
		return
	var update_delta := maxf(_perception_tick_accumulator, delta)
	_perception_tick_accumulator = 0.0
	_update_perception(update_delta, active_subject_ids)


func get_sim_state() -> Dictionary:
	return _last_sim_state.duplicate(true)


func get_latest_results_for_subject(subject) -> Array[Dictionary]:
	var subject_id := _actor_id_from_value(subject)
	if subject_id.is_empty():
		return []
	var raw_results: Array = _latest_results_by_subject_id.get(subject_id, []) if _latest_results_by_subject_id.get(subject_id, []) is Array else []
	var results: Array[Dictionary] = []
	for result in raw_results:
		if result is Dictionary:
			results.append((result as Dictionary).duplicate(true))
	return results


func get_latest_result(observer, subject) -> Dictionary:
	var observer_id := _actor_id_from_value(observer)
	var subject_id := _actor_id_from_value(subject)
	if observer_id.is_empty() or subject_id.is_empty():
		return {}
	var result: Dictionary = _latest_results_by_pair.get(_pair_key(observer_id, subject_id), {}) if _latest_results_by_pair.get(_pair_key(observer_id, subject_id), {}) is Dictionary else {}
	return result.duplicate(true)


func evaluate_observer(observer, subject) -> Dictionary:
	var observer_id := _actor_id_from_value(observer)
	var subject_id := _actor_id_from_value(subject)
	if observer_id.is_empty() or subject_id.is_empty():
		return {}
	var records := _get_population_records()
	return _evaluate_observer(observer_id, subject_id, records, -1.0, debug_show_los_rays)


func is_clearly_seen_by_anyone(subject) -> bool:
	for result in get_latest_results_for_subject(subject):
		if bool(result.get("clearly_seen", false)):
			return true
	return false


func _update_perception(delta: float, active_subjects: Array) -> void:
	if not _try_initialize():
		return
	var records := _get_population_records()
	var observer_candidates := _get_alive_non_party_observer_ids(records)
	var observer_index := _observer_spatial_index(observer_candidates, records)
	var local_lights := _get_local_lights()
	var active_keys: Dictionary = {}
	var next_by_subject: Dictionary = {}
	var next_by_pair: Dictionary = {}
	var debug_segments: Array[Dictionary] = []
	for subject_value in active_subjects:
		var subject_id := _actor_id_from_value(subject_value)
		if subject_id.is_empty() or not records.has(subject_id):
			continue
		var subject_record: Dictionary = records.get(subject_id, {}) if records.get(subject_id, {}) is Dictionary else {}
		if not _can_participate_in_perception(subject_record):
			continue
		var subject_results: Array[Dictionary] = []
		var subject_light_exposure := _calculate_light_exposure(subject_id, local_lights)
		var best_training_pressure := 0.0
		for observer_id in _get_observers_for_subject(subject_id, observer_index, records):
			var key := _pair_key(observer_id, subject_id)
			var result := _evaluate_observer(observer_id, subject_id, records, subject_light_exposure, debug_show_los_rays)
			if result.is_empty():
				continue
			result = _apply_suspicion_memory(delta, key, result)
			_award_observer_perception_xp(delta, observer_id, result, records)
			var training_pressure := _get_sneaking_training_pressure(result)
			result["sneak_training_pressure"] = training_pressure
			best_training_pressure = maxf(best_training_pressure, training_pressure)
			active_keys[key] = true
			next_by_pair[key] = result
			subject_results.append(result)
			_update_indicator(key, observer_id, subject_id, result)
			if debug_show_los_rays:
				var segments: Array = result.get("sample_segments", []) if result.get("sample_segments", []) is Array else []
				for segment in segments:
					if segment is Dictionary:
						debug_segments.append(segment)
		var subject_sneaking_xp := _get_sneaking_risk_xp(delta, best_training_pressure, _get_sneaking_activity_multiplier(subject_id, subject_record))
		if subject_sneaking_xp > 0.0:
			_add_skill_xp(subject_id, SkillRules.SUBTERFUGE_SNEAKING, subject_sneaking_xp, records)
		next_by_subject[subject_id] = subject_results
	_latest_results_by_subject_id = next_by_subject
	_latest_results_by_pair = next_by_pair
	_last_sim_state = {"active_subject_count": active_subjects.size(), "observer_count": observer_candidates.size(), "pair_count": next_by_pair.size()}
	_remove_inactive_indicators(active_keys)
	_update_debug_rays(debug_segments)


func _clear_perception_state() -> void:
	if _latest_results_by_subject_id.is_empty() and _latest_results_by_pair.is_empty() and _indicators.is_empty() and _suspicion_states.is_empty():
		return
	_latest_results_by_subject_id.clear()
	_latest_results_by_pair.clear()
	_suspicion_states.clear()
	_remove_inactive_indicators({})
	_update_debug_rays([])


func _evaluate_observer(observer_id: String, subject_id: String, records: Dictionary, cached_light_exposure := -1.0, collect_debug_segments := false) -> Dictionary:
	if observer_id.is_empty() or subject_id.is_empty() or observer_id == subject_id:
		return {}
	var observer_record: Dictionary = records.get(observer_id, {}) if records.get(observer_id, {}) is Dictionary else {}
	var subject_record: Dictionary = records.get(subject_id, {}) if records.get(subject_id, {}) is Dictionary else {}
	if observer_record.is_empty() or subject_record.is_empty():
		return {}
	var sample_positions := _stealth_sample_positions(subject_id, subject_record)
	if sample_positions.is_empty():
		return {}
	var eye_position := _perception_eye_position(observer_id, observer_record)
	var visible_samples := 0
	var cone_samples := 0
	var segments: Array[Dictionary] = []
	var max_distance_factor := 0.0
	var nearest_visible_distance := INF
	var ignored_actor_ids := {}
	ignored_actor_ids[observer_id] = true
	ignored_actor_ids[subject_id] = true
	for sample_position in sample_positions:
		var to_sample := sample_position - eye_position
		var distance := to_sample.length()
		var in_cone := distance <= view_distance and _is_in_front_cone(observer_id, observer_record, to_sample)
		var clear_los := false
		if in_cone:
			cone_samples += 1
			clear_los = _has_clear_ray(eye_position, sample_position, ignored_actor_ids)
			if clear_los:
				visible_samples += 1
				max_distance_factor = maxf(max_distance_factor, _distance_factor(distance, view_distance))
				nearest_visible_distance = minf(nearest_visible_distance, distance)
		if collect_debug_segments:
			segments.append({"from": eye_position, "to": sample_position, "visible": in_cone and clear_los})
	var los_fraction := float(visible_samples) / float(sample_positions.size())
	var light_exposure := cached_light_exposure
	if light_exposure < 0.0:
		light_exposure = _calculate_light_exposure(subject_id, _get_local_lights())
	var subject_sneaking := WORLD_ACTOR_RULES.get_stat_value(subject_record, "stealth")
	var observer_perception := WORLD_ACTOR_RULES.get_stat_value(observer_record, "perception")
	var posture := _get_posture_visibility(_record_is_sneaking(subject_record), subject_sneaking)
	var base_visibility_score := los_fraction * light_exposure * max_distance_factor * posture
	var skill_gap := subject_sneaking - observer_perception
	var contested_multiplier := exp(-maxf(skill_gap, 0.0) / maxf(sneak_contested_skill_curve, 0.001))
	if skill_gap < 0.0:
		contested_multiplier = 1.0 + SkillRules.get_diminishing_bonus(-skill_gap, 0.65, 35.0)
	var close_ratio := 0.0
	if nearest_visible_distance < INF:
		close_ratio = clampf(1.0 - (nearest_visible_distance - 1.0) / 3.0, 0.0, 1.0)
	var close_reveal := los_fraction * light_exposure * posture * close_ratio * _get_close_reveal_multiplier(subject_sneaking, observer_perception)
	var novice_visibility_floor := _get_novice_clear_los_visibility_floor(subject_sneaking, observer_perception, los_fraction, max_distance_factor, close_ratio)
	var visibility_score := clampf(maxf(base_visibility_score * contested_multiplier + close_reveal, novice_visibility_floor), 0.0, 1.0)
	return {
		"observer_id": observer_id,
		"subject_id": subject_id,
		"line_of_sight_fraction": los_fraction,
		"cone_fraction": float(cone_samples) / float(sample_positions.size()),
		"light_exposure": light_exposure,
		"distance_factor": max_distance_factor,
		"posture_visibility": posture,
		"observer_perception": observer_perception,
		"subject_sneaking": subject_sneaking,
		"contested_multiplier": contested_multiplier,
		"close_reveal": close_reveal,
		"novice_visibility_floor": novice_visibility_floor,
		"base_visibility_score": base_visibility_score,
		"visibility_score": visibility_score,
		"clearly_seen": visibility_score >= clear_seen_threshold,
		"partially_seen": visibility_score >= partial_seen_threshold and visibility_score < clear_seen_threshold,
		"sample_segments": segments,
	}


func _get_active_sneaking_subject_ids() -> Array[String]:
	var records := _get_population_records()
	var result: Array[String] = []
	for actor_id_value in records.keys():
		var actor_id := str(actor_id_value).strip_edges()
		if actor_id.is_empty():
			continue
		var record: Dictionary = records.get(actor_id, {}) if records.get(actor_id, {}) is Dictionary else {}
		if _record_is_sneaking(record) and _can_participate_in_perception(record):
			result.append(actor_id)
	return result


func _get_alive_non_party_observer_ids(records: Dictionary) -> Array[String]:
	var observers: Array[String] = []
	for actor_id_value in records.keys():
		var actor_id := str(actor_id_value).strip_edges()
		var record: Dictionary = records.get(actor_id, {}) if records.get(actor_id, {}) is Dictionary else {}
		if actor_id.is_empty() or bool(record.get("player_party_member", false)) or bool(record.get("player_controllable", false)):
			continue
		if _can_participate_in_perception(record):
			observers.append(actor_id)
	return observers


func _observer_spatial_index(observer_candidates: Array[String], records: Dictionary) -> Dictionary:
	var bin_size := maxf(observer_radius, 1.0)
	var bins := {}
	for observer_id in observer_candidates:
		var observer_record: Dictionary = records.get(observer_id, {}) if records.get(observer_id, {}) is Dictionary else {}
		if not _can_participate_in_perception(observer_record):
			continue
		var position := _actor_position(observer_id, observer_record)
		var bin_key := _observer_bin_key(position, bin_size)
		var bucket: Array = bins.get(bin_key, []) if bins.get(bin_key, []) is Array else []
		bucket.append(observer_id)
		bins[bin_key] = bucket
	return {"bins": bins, "bin_size": bin_size}


func _get_observers_for_subject(subject_id: String, observer_index: Dictionary, records: Dictionary) -> Array[String]:
	var observers: Array[String] = []
	var subject_record: Dictionary = records.get(subject_id, {}) if records.get(subject_id, {}) is Dictionary else {}
	var subject_position := _actor_position(subject_id, subject_record)
	var observer_radius_squared := observer_radius * observer_radius
	var bins: Dictionary = observer_index.get("bins", {}) if observer_index.get("bins", {}) is Dictionary else {}
	var bin_size := maxf(float(observer_index.get("bin_size", observer_radius)), 1.0)
	var center_x := floori(subject_position.x / bin_size)
	var center_z := floori(subject_position.z / bin_size)
	var candidates: Array[Dictionary] = []
	var checked_count := 0
	var check_cap := maxi(1, max_observer_candidate_checks_per_subject)
	for x_offset in range(-1, 2):
		for z_offset in range(-1, 2):
			var bucket_key := _observer_bin_key_from_coords(center_x + int(x_offset), center_z + int(z_offset))
			var bucket: Array = bins.get(bucket_key, []) if bins.get(bucket_key, []) is Array else []
			for observer_id_value in bucket:
				if checked_count >= check_cap:
					break
				checked_count += 1
				var observer_id := str(observer_id_value).strip_edges()
				if observer_id.is_empty() or observer_id == subject_id:
					continue
				var observer_record: Dictionary = records.get(observer_id, {}) if records.get(observer_id, {}) is Dictionary else {}
				if not _can_participate_in_perception(observer_record):
					continue
				var distance_squared := _actor_position(observer_id, observer_record).distance_squared_to(subject_position)
				if distance_squared > observer_radius_squared:
					continue
				candidates.append({"observer_id": observer_id, "distance_squared": distance_squared})
			if checked_count >= check_cap:
				break
		if checked_count >= check_cap:
			break
	candidates.sort_custom(func(first: Dictionary, second: Dictionary) -> bool:
		var first_distance := float(first.get("distance_squared", 0.0))
		var second_distance := float(second.get("distance_squared", 0.0))
		if is_equal_approx(first_distance, second_distance):
			return str(first.get("observer_id", "")) < str(second.get("observer_id", ""))
		return first_distance < second_distance
	)
	var observer_cap := mini(maxi(1, max_observers_per_subject), candidates.size())
	for index in range(observer_cap):
		observers.append(str(candidates[index].get("observer_id", "")))
	return observers


func _observer_bin_key(position: Vector3, bin_size: float) -> String:
	return _observer_bin_key_from_coords(floori(position.x / maxf(bin_size, 1.0)), floori(position.z / maxf(bin_size, 1.0)))


func _observer_bin_key_from_coords(x: int, z: int) -> String:
	return "%d:%d" % [x, z]


func _award_observer_perception_xp(delta: float, observer_id: String, result: Dictionary, records: Dictionary) -> void:
	if bool(result.get("clearly_seen", false)):
		_add_skill_xp(observer_id, SkillRules.ATTRIBUTE_PERCEPTION, SNEAK_DETECTION_PERCEPTION_XP_PER_SECOND * delta, records)
	elif bool(result.get("partially_seen", false)):
		_add_skill_xp(observer_id, SkillRules.ATTRIBUTE_PERCEPTION, SNEAK_DETECTION_PERCEPTION_XP_PER_SECOND * 0.35 * delta, records)


func _get_sneaking_training_pressure(result: Dictionary) -> float:
	if bool(result.get("clearly_seen", false)):
		return 0.0
	if float(result.get("line_of_sight_fraction", 0.0)) <= 0.0 or float(result.get("cone_fraction", 0.0)) <= 0.0:
		return 0.0
	var score := float(result.get("visibility_score", 0.0))
	if score <= SNEAKING_RISK_SCORE_FLOOR:
		return 0.0
	var visibility_pressure := clampf(score / maxf(clear_seen_threshold, 0.001), 0.1, 1.0)
	var subject_sneaking := float(result.get("subject_sneaking", SkillRules.DEFAULT_LEVEL))
	var observer_perception := float(result.get("observer_perception", SkillRules.DEFAULT_LEVEL))
	var observer_relevance := _get_sneaking_observer_relevance(subject_sneaking, observer_perception)
	if observer_relevance <= 0.0:
		return 0.0
	var challenge_gap := observer_perception - subject_sneaking
	var challenge_pressure := 1.0
	if challenge_gap >= 0.0:
		challenge_pressure += SkillRules.get_diminishing_bonus(challenge_gap, 0.55, 35.0)
	var pressure := visibility_pressure * challenge_pressure * observer_relevance
	var novice_floor_ratio := clampf((SNEAKING_NOVICE_PRESSURE_FLOOR_END_LEVEL - subject_sneaking) / maxf(SNEAKING_NOVICE_PRESSURE_FLOOR_END_LEVEL - float(SkillRules.DEFAULT_LEVEL), 0.001), 0.0, 1.0)
	var novice_floor := SNEAKING_NOVICE_MIN_TRAINING_PRESSURE * novice_floor_ratio * observer_relevance
	return maxf(pressure, novice_floor)


func _get_sneaking_observer_relevance(subject_sneaking: float, observer_perception: float) -> float:
	var overmatch := subject_sneaking - observer_perception
	if overmatch <= SNEAKING_OBSERVER_GRACE_LEVELS:
		return 1.0
	var fade_ratio := clampf((overmatch - SNEAKING_OBSERVER_GRACE_LEVELS) / maxf(SNEAKING_OBSERVER_FADE_LEVELS, 0.001), 0.0, 1.0)
	return pow(1.0 - fade_ratio, 1.5)


func _get_sneaking_risk_xp(delta: float, raw_pressure: float, activity_multiplier: float) -> float:
	if raw_pressure <= 0.0 or activity_multiplier <= 0.0:
		return 0.0
	var capped_pressure := SNEAKING_RISK_PRESSURE_CAP * (1.0 - exp(-raw_pressure / maxf(SNEAKING_RISK_PRESSURE_CAP, 0.001)))
	return SNEAKING_RISK_XP_PER_SECOND * capped_pressure * activity_multiplier * delta


func _get_sneaking_activity_multiplier(_subject_id: String, record: Dictionary) -> float:
	if not _record_is_sneaking(record):
		return 0.0
	var locomotion: Dictionary = record.get("locomotion_state", {}) if record.get("locomotion_state", {}) is Dictionary else {}
	var horizontal_speed := float(locomotion.get("horizontal_speed", 0.0))
	if horizontal_speed >= SNEAKING_MOVING_SPEED_THRESHOLD:
		return 1.0
	return SNEAKING_IDLE_XP_MULTIPLIER


func _get_posture_visibility(is_sneaking: bool, subject_sneaking: float) -> float:
	if not is_sneaking:
		return standing_posture_visibility
	var skill_ratio := _get_sneak_mastery_ratio(subject_sneaking)
	var mastery := pow(skill_ratio, maxf(sneak_posture_skill_curve, 0.001))
	return lerpf(novice_sneak_posture_visibility, master_sneak_posture_visibility, mastery)


func _get_close_reveal_multiplier(subject_sneaking: float, observer_perception: float) -> float:
	var skill_ratio := _get_sneak_mastery_ratio(subject_sneaking)
	var mastery := pow(skill_ratio, maxf(close_reveal_skill_curve, 0.001))
	var sneak_multiplier := lerpf(novice_close_reveal_multiplier, master_close_reveal_multiplier, mastery)
	var observer_pressure := 1.0 + SkillRules.get_diminishing_bonus(observer_perception, 0.22, 35.0)
	return sneak_multiplier * observer_pressure


func _get_novice_clear_los_visibility_floor(subject_sneaking: float, observer_perception: float, los_fraction: float, distance_factor: float, close_ratio: float) -> float:
	if los_fraction <= 0.0:
		return 0.0
	var novice_factor := clampf((10.0 - subject_sneaking) / 9.0, 0.0, 1.0)
	if novice_factor <= 0.0:
		return 0.0
	var observer_pressure := 0.85 + SkillRules.get_diminishing_bonus(observer_perception, 0.35, 35.0)
	var floor_score := novice_clear_los_visibility_floor * novice_factor * los_fraction * distance_factor * observer_pressure
	floor_score += 0.12 * novice_factor * los_fraction * close_ratio * observer_pressure
	return floor_score


func _get_sneak_mastery_ratio(subject_sneaking: float) -> float:
	return clampf((subject_sneaking - float(SkillRules.DEFAULT_LEVEL)) / 79.0, 0.0, 1.0)


func _apply_suspicion_memory(delta: float, key: String, result: Dictionary) -> Dictionary:
	var state: Dictionary = _suspicion_states.get(key, {}) if _suspicion_states.get(key, {}) is Dictionary else {}
	result = _apply_sustained_moving_exposure(delta, result, state)
	var suspicion := float(state.get("suspicion", 0.0))
	var threshold := _get_suspicion_threshold(result)
	if bool(result.get("clearly_seen", false)):
		suspicion = threshold
	elif bool(result.get("partially_seen", false)):
		suspicion += delta * _get_suspicion_charge_multiplier(result)
	else:
		suspicion = maxf(0.0, suspicion - delta * 1.35)
	var escalated := not bool(result.get("clearly_seen", false)) and bool(result.get("partially_seen", false)) and suspicion >= threshold
	if escalated:
		result["visibility_score"] = maxf(float(result.get("visibility_score", 0.0)), clear_seen_threshold)
		result["clearly_seen"] = true
		result["partially_seen"] = false
	result["suspicion_escalated"] = escalated
	result["suspicion_progress"] = clampf(suspicion / maxf(threshold, 0.001), 0.0, 1.0)
	result["suspicion_threshold"] = threshold
	state["suspicion"] = minf(suspicion, threshold)
	_suspicion_states[key] = state
	return result


func _apply_sustained_moving_exposure(delta: float, result: Dictionary, state: Dictionary) -> Dictionary:
	var exposure := float(state.get("sustained_moving_exposure", 0.0))
	var cone_fraction := float(result.get("cone_fraction", 0.0))
	var los_fraction := float(result.get("line_of_sight_fraction", 0.0))
	if cone_fraction <= 0.0 or los_fraction <= 0.0:
		exposure = 0.0
	elif _is_sustained_moving_exposure_active(result):
		exposure = clampf(exposure + delta * _get_sustained_moving_exposure_charge_rate(result), 0.0, 1.0)
	else:
		exposure = maxf(0.0, exposure - delta * SUSTAINED_MOVING_EXPOSURE_IDLE_DECAY_PER_SECOND)
	state["sustained_moving_exposure"] = exposure
	var bonus := exposure * SUSTAINED_MOVING_EXPOSURE_SCORE_CAP
	if bonus > 0.0:
		if not result.has("raw_visibility_score"):
			result["raw_visibility_score"] = float(result.get("visibility_score", 0.0))
		var visibility_score := clampf(float(result.get("visibility_score", 0.0)) + bonus, 0.0, 1.0)
		result["visibility_score"] = visibility_score
		result["clearly_seen"] = visibility_score >= clear_seen_threshold
		result["partially_seen"] = visibility_score >= partial_seen_threshold and visibility_score < clear_seen_threshold
	result["sustained_moving_exposure"] = exposure
	result["sustained_moving_exposure_bonus"] = bonus
	result["sustained_moving_exposure_active"] = _is_sustained_moving_exposure_active(result)
	return result


func _is_sustained_moving_exposure_active(result: Dictionary) -> bool:
	if float(result.get("cone_fraction", 0.0)) <= 0.0 or float(result.get("line_of_sight_fraction", 0.0)) <= 0.0:
		return false
	if float(result.get("light_exposure", 0.0)) <= SUSTAINED_MOVING_EXPOSURE_MIN_LIGHT:
		return false
	var subject_id := str(result.get("subject_id", "")).strip_edges()
	var record := _population_record(subject_id)
	return _get_sneaking_activity_multiplier(subject_id, record) >= 1.0


func _get_sustained_moving_exposure_charge_rate(result: Dictionary) -> float:
	var light_pressure := clampf((float(result.get("light_exposure", 0.0)) - SUSTAINED_MOVING_EXPOSURE_MIN_LIGHT) / 0.7, 0.0, 1.0)
	var sight_pressure := clampf(float(result.get("line_of_sight_fraction", 0.0)) * float(result.get("distance_factor", 0.0)), 0.0, 1.0)
	var subject_sneaking := float(result.get("subject_sneaking", SkillRules.DEFAULT_LEVEL))
	var observer_perception := float(result.get("observer_perception", SkillRules.DEFAULT_LEVEL))
	var mastery_resistance := lerpf(1.25, 0.18, _get_sneak_mastery_ratio(subject_sneaking))
	var observer_pressure := 0.75 + SkillRules.get_diminishing_bonus(observer_perception, 0.85, 55.0)
	var challenge_ratio := clampf((observer_perception + 20.0) / maxf(subject_sneaking, 1.0), 0.2, 1.4)
	return light_pressure * sight_pressure * mastery_resistance * observer_pressure * challenge_ratio


func _get_suspicion_threshold(result: Dictionary) -> float:
	var subject_sneaking := float(result.get("subject_sneaking", SkillRules.DEFAULT_LEVEL))
	var observer_perception := float(result.get("observer_perception", SkillRules.DEFAULT_LEVEL))
	var mastery := pow(_get_sneak_mastery_ratio(subject_sneaking), 0.7)
	var threshold := lerpf(novice_suspicion_seconds, master_suspicion_seconds, mastery)
	var observer_pressure := 1.0 + SkillRules.get_diminishing_bonus(observer_perception, 0.45, 35.0)
	return maxf(0.25, threshold / observer_pressure)


func _get_suspicion_charge_multiplier(result: Dictionary) -> float:
	var score := float(result.get("visibility_score", 0.0))
	var score_pressure := clampf((score - partial_seen_threshold) / maxf(clear_seen_threshold - partial_seen_threshold, 0.001), 0.0, 1.0)
	var subject_sneaking := float(result.get("subject_sneaking", SkillRules.DEFAULT_LEVEL))
	var novice_pressure := clampf((20.0 - subject_sneaking) / 19.0, 0.0, 1.0)
	return lerpf(0.85, 1.45, score_pressure) * lerpf(1.25, 1.0, _get_sneak_mastery_ratio(subject_sneaking)) * lerpf(1.0, 1.2, novice_pressure)


func _is_in_front_cone(observer_id: String, observer_record: Dictionary, to_sample: Vector3) -> bool:
	var flat_to_sample := Vector3(to_sample.x, 0.0, to_sample.z)
	if flat_to_sample.length_squared() <= 0.0001:
		return true
	var forward := _perception_forward_vector(observer_id, observer_record)
	forward.y = 0.0
	if forward.length_squared() <= 0.0001:
		return false
	var dot := forward.normalized().dot(flat_to_sample.normalized())
	var half_angle := deg_to_rad(view_cone_degrees) * 0.5
	return dot >= cos(half_angle)


func _has_clear_ray(from: Vector3, to: Vector3, ignored_actor_ids: Dictionary = {}) -> bool:
	var world := _get_world_3d()
	if world == null:
		return false
	var ray_exclusions: Array[RID] = []
	for _hit_index in range(8):
		var query := PhysicsRayQueryParameters3D.create(from, to)
		query.exclude = ray_exclusions
		query.collide_with_bodies = true
		query.collide_with_areas = false
		var hit := world.direct_space_state.intersect_ray(query)
		if hit.is_empty():
			return true
		if not _ray_hit_ignored_actor(hit, ignored_actor_ids):
			return false
		var rid: RID = hit.get("rid", RID())
		if rid.is_valid():
			ray_exclusions.append(rid)
			continue
		var collider = hit.get("collider", null)
		if collider is CollisionObject3D:
			ray_exclusions.append((collider as CollisionObject3D).get_rid())
			continue
		return true
	return false


func _ray_hit_ignored_actor(hit: Dictionary, ignored_actor_ids: Dictionary) -> bool:
	if ignored_actor_ids.is_empty():
		return false
	var collider = hit.get("collider", null)
	var actor_id := _collider_actor_id(collider)
	return not actor_id.is_empty() and ignored_actor_ids.has(actor_id)


func _collider_actor_id(collider) -> String:
	var node := collider as Node
	var depth := 0
	while node != null and depth < 6:
		if node.has_meta("actor_id"):
			return str(node.get_meta("actor_id")).strip_edges()
		node = node.get_parent()
		depth += 1
	return ""


func _get_world_3d() -> World3D:
	if root_scene is Node3D:
		return (root_scene as Node3D).get_world_3d()
	var viewport := get_viewport()
	return viewport.world_3d if viewport != null else null


func _distance_factor(distance: float, max_distance: float) -> float:
	var ratio := clampf(distance / maxf(max_distance, 0.001), 0.0, 1.0)
	return clampf(1.0 - ratio * 0.55, 0.0, 1.0)


func _calculate_light_exposure(subject_id: String, local_lights: Array[Light3D]) -> float:
	var exposure := _get_day_night_exposure()
	var sample_position := _stealth_light_sample_position(subject_id, _population_record(subject_id))
	var ignored_actor_ids := {}
	ignored_actor_ids[subject_id] = true
	for light in _stealth_light_candidates(sample_position, local_lights):
		exposure += _get_light_contribution(light, sample_position, ignored_actor_ids)
	return clampf(exposure, 0.08, 1.35)


func _stealth_light_candidates(sample_position: Vector3, local_lights: Array[Light3D]) -> Array[Light3D]:
	var candidates: Array[Dictionary] = []
	for light in local_lights:
		if light == null or not is_instance_valid(light) or not light.visible or light.light_energy <= 0.0:
			continue
		candidates.append({"light": light, "distance_squared": light.global_position.distance_squared_to(sample_position)})
	candidates.sort_custom(func(first: Dictionary, second: Dictionary) -> bool:
		return float(first.get("distance_squared", 0.0)) < float(second.get("distance_squared", 0.0))
	)
	var result: Array[Light3D] = []
	var cap := mini(maxi(1, max_stealth_lights_per_subject), candidates.size())
	for index in range(cap):
		var light = candidates[index].get("light", null)
		if light is Light3D:
			result.append(light as Light3D)
	return result


func _get_day_night_exposure() -> float:
	var lighting := _day_night_lighting_controller()
	if lighting != null and lighting.has_method("get_stealth_ambient_visibility"):
		return float(lighting.call("get_stealth_ambient_visibility"))
	var world_time := _world_time_controller()
	if world_time == null or not world_time.has_method("get_day_fraction"):
		return 0.75
	var day_fraction := float(world_time.call("get_day_fraction"))
	var sun_altitude := sin((day_fraction - 0.25) * TAU)
	return lerpf(0.16, 0.95, _smoothstep(-0.08, 0.35, sun_altitude))


func _get_local_lights(force_refresh := false) -> Array[Light3D]:
	if _local_light_cache_initialized and not force_refresh:
		return _local_light_cache
	_refresh_local_light_cache()
	return _local_light_cache


func _refresh_local_light_cache() -> void:
	var lights: Array[Light3D] = []
	var seen: Dictionary = {}
	if is_inside_tree():
		for node in get_tree().get_nodes_in_group("stealth_light_source"):
			if node is Light3D and is_instance_valid(node) and not seen.has(node.get_instance_id()):
				lights.append(node as Light3D)
				seen[node.get_instance_id()] = true
				if lights.size() >= max_cached_stealth_lights:
					break
	_local_light_cache = lights
	_local_light_cache_initialized = true


func _get_light_contribution(light: Light3D, sample_position: Vector3, ignored_actor_ids: Dictionary) -> float:
	if light == null or not is_instance_valid(light) or not light.visible or light.light_energy <= 0.0:
		return 0.0
	if light is OmniLight3D:
		var omni := light as OmniLight3D
		var distance := light.global_position.distance_to(sample_position)
		if distance > omni.omni_range:
			return 0.0
		if not _has_clear_ray(light.global_position, sample_position, ignored_actor_ids):
			return 0.0
		var ratio := clampf(distance / maxf(omni.omni_range, 0.001), 0.0, 1.0)
		var attenuation := pow(1.0 - ratio, maxf(0.35, omni.omni_attenuation))
		return light.light_energy * attenuation * 0.65
	if light is SpotLight3D:
		var spot := light as SpotLight3D
		var to_sample := sample_position - light.global_position
		var distance := to_sample.length()
		if distance > spot.spot_range or distance <= 0.001:
			return 0.0
		var forward := -light.global_transform.basis.z.normalized()
		var dot := forward.dot(to_sample.normalized())
		if dot < cos(deg_to_rad(spot.spot_angle)):
			return 0.0
		if not _has_clear_ray(light.global_position, sample_position, ignored_actor_ids):
			return 0.0
		var distance_ratio := clampf(distance / maxf(spot.spot_range, 0.001), 0.0, 1.0)
		var distance_attenuation := pow(1.0 - distance_ratio, maxf(0.35, spot.spot_attenuation))
		var angle_attenuation := pow(clampf(dot, 0.0, 1.0), maxf(0.35, spot.spot_angle_attenuation))
		return light.light_energy * distance_attenuation * angle_attenuation * 0.75
	return 0.0


func _update_indicator(key: String, observer_id: String, subject_id: String, result: Dictionary) -> void:
	if root_scene == null:
		return
	var indicator := _indicators.get(key, null) as Node3D
	if indicator == null or not is_instance_valid(indicator):
		indicator = _create_indicator()
		_indicators[key] = indicator
	var score := float(result.get("visibility_score", 0.0))
	indicator.visible = true
	indicator.global_position = _stealth_indicator_position(observer_id, _population_record(observer_id))
	var arrow_root := indicator.get_node_or_null("ArrowRoot") as Node3D
	if arrow_root != null:
		var subject_position := _actor_position(subject_id, _population_record(subject_id))
		var look_target := Vector3(subject_position.x, indicator.global_position.y, subject_position.z)
		if indicator.global_position.distance_squared_to(look_target) > 0.001:
			arrow_root.look_at(look_target, Vector3.UP)
	var color := Color(0.2, 0.34, 0.72, 0.42)
	var eye_scale_y := 0.035
	if bool(result.get("clearly_seen", false)):
		color = Color(1.0, 0.2, 0.08, 1.0)
		eye_scale_y = 0.13
	elif bool(result.get("partially_seen", false)):
		color = Color(1.0, 0.68, 0.16, 0.82)
		eye_scale_y = 0.075
	_set_indicator_color(indicator, color, eye_scale_y, score)
	var camera := _current_camera()
	if camera != null:
		var eye_root := indicator.get_node_or_null("EyeRoot") as Node3D
		if eye_root != null:
			eye_root.look_at(camera.global_position, Vector3.UP)


func _create_indicator() -> Node3D:
	var root := Node3D.new()
	root.name = "SneakObserverIndicator"
	root.top_level = true
	root.visible = false
	root_scene.add_child(root)
	var eye_root := Node3D.new()
	eye_root.name = "EyeRoot"
	root.add_child(eye_root)
	var eye := MeshInstance3D.new()
	eye.name = "Eye"
	var eye_mesh := SphereMesh.new()
	eye_mesh.radius = 1.0
	eye_mesh.height = 2.0
	eye_mesh.radial_segments = 24
	eye_mesh.rings = 12
	eye.mesh = eye_mesh
	eye.scale = Vector3(0.28, 0.08, 0.055)
	eye.material_override = _make_unshaded_material(Color(0.2, 0.34, 0.72, 0.42))
	eye_root.add_child(eye)
	var pupil := MeshInstance3D.new()
	pupil.name = "Pupil"
	var pupil_mesh := SphereMesh.new()
	pupil_mesh.radius = 1.0
	pupil_mesh.height = 2.0
	pupil_mesh.radial_segments = 16
	pupil_mesh.rings = 8
	pupil.mesh = pupil_mesh
	pupil.scale = Vector3(0.07, 0.07, 0.03)
	pupil.position = Vector3(0.0, 0.0, -0.055)
	pupil.material_override = _make_unshaded_material(Color(0.02, 0.015, 0.01, 0.42))
	eye_root.add_child(pupil)
	var arrow_root := Node3D.new()
	arrow_root.name = "ArrowRoot"
	root.add_child(arrow_root)
	var shaft := MeshInstance3D.new()
	shaft.name = "ArrowShaft"
	var shaft_mesh := BoxMesh.new()
	shaft_mesh.size = Vector3(0.055, 0.055, 0.78)
	shaft.mesh = shaft_mesh
	shaft.position = Vector3(0.0, -0.26, -0.52)
	shaft.material_override = _make_unshaded_material(Color(0.2, 0.34, 0.72, 0.30))
	arrow_root.add_child(shaft)
	var head := MeshInstance3D.new()
	head.name = "ArrowHead"
	var head_mesh := BoxMesh.new()
	head_mesh.size = Vector3(0.18, 0.09, 0.18)
	head.mesh = head_mesh
	head.position = Vector3(0.0, -0.26, -0.94)
	head.rotation.y = deg_to_rad(45.0)
	head.material_override = _make_unshaded_material(Color(0.2, 0.34, 0.72, 0.30))
	arrow_root.add_child(head)
	return root


func _set_indicator_color(indicator: Node3D, color: Color, eye_scale_y: float, score: float) -> void:
	var eye := indicator.get_node_or_null("EyeRoot/Eye") as MeshInstance3D
	if eye != null:
		eye.scale.y = eye_scale_y
		_set_unshaded_material_color(eye, color)
	var pupil := indicator.get_node_or_null("EyeRoot/Pupil") as MeshInstance3D
	if pupil != null:
		pupil.visible = score >= partial_seen_threshold
		_set_unshaded_material_color(pupil, Color(0.02, 0.015, 0.01, color.a))
	var arrow_color := Color(color.r, color.g, color.b, clampf(color.a * 0.72, 0.18, 0.86))
	for node_path in ["ArrowRoot/ArrowShaft", "ArrowRoot/ArrowHead"]:
		var mesh_instance := indicator.get_node_or_null(node_path) as MeshInstance3D
		if mesh_instance != null:
			mesh_instance.visible = score >= 0.02
			_set_unshaded_material_color(mesh_instance, arrow_color)


func _set_unshaded_material_color(mesh_instance: MeshInstance3D, color: Color) -> void:
	var material := mesh_instance.material_override as StandardMaterial3D
	if material == null:
		mesh_instance.material_override = _make_unshaded_material(color)
		return
	material.albedo_color = color


func _make_unshaded_material(color: Color) -> StandardMaterial3D:
	var material := StandardMaterial3D.new()
	material.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	material.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	material.albedo_color = color
	material.no_depth_test = true
	return material


func _remove_inactive_indicators(active_keys: Dictionary) -> void:
	for key in _indicators.keys():
		if active_keys.has(key):
			continue
		var indicator := _indicators.get(key, null) as Node3D
		if indicator != null and is_instance_valid(indicator):
			indicator.queue_free()
		_indicators.erase(key)
	for key in _suspicion_states.keys():
		if not active_keys.has(key):
			_suspicion_states.erase(key)


func _ensure_debug_ray_mesh() -> void:
	if _debug_ray_mesh_instance != null and is_instance_valid(_debug_ray_mesh_instance):
		return
	if root_scene == null:
		return
	_debug_ray_material.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	_debug_ray_material.vertex_color_use_as_albedo = true
	_debug_ray_material.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	_debug_ray_mesh_instance = MeshInstance3D.new()
	_debug_ray_mesh_instance.name = "SneakDebugLosRays"
	_debug_ray_mesh_instance.mesh = _debug_ray_mesh
	_debug_ray_mesh_instance.top_level = true
	_debug_ray_mesh_instance.visible = false
	root_scene.add_child(_debug_ray_mesh_instance)


func _update_debug_rays(segments: Array[Dictionary]) -> void:
	_ensure_debug_ray_mesh()
	if _debug_ray_mesh_instance == null:
		return
	_debug_ray_mesh.clear_surfaces()
	_debug_ray_mesh_instance.visible = debug_show_los_rays and not segments.is_empty()
	if not _debug_ray_mesh_instance.visible:
		return
	_debug_ray_mesh.surface_begin(Mesh.PRIMITIVE_LINES, _debug_ray_material)
	for segment in segments:
		var color := Color(0.2, 1.0, 0.35, 0.9) if bool(segment.get("visible", false)) else Color(1.0, 0.18, 0.12, 0.75)
		_debug_ray_mesh.surface_set_color(color)
		_debug_ray_mesh.surface_add_vertex(segment.get("from", Vector3.ZERO))
		_debug_ray_mesh.surface_add_vertex(segment.get("to", Vector3.ZERO))
	_debug_ray_mesh.surface_end()


func _try_initialize() -> bool:
	if root_scene == null:
		root_scene = get_parent().get_parent() if get_parent() != null else null
	if root_scene == null or not is_inside_tree():
		return false
	if not _initialized:
		_ensure_debug_ray_mesh()
		_get_local_lights(true)
		_initialized = true
	return true


func _can_participate_in_perception(record: Dictionary) -> bool:
	return WORLD_ACTOR_RULES.can_participate(record)


func _record_is_sneaking(record: Dictionary) -> bool:
	return int(record.get("movement_mode", 0)) == MOVEMENT_MODE_SNEAK


func _actor_id_from_value(value) -> String:
	if value is String or value is StringName:
		return str(value).strip_edges()
	if value is Dictionary:
		return str((value as Dictionary).get("actor_id", (value as Dictionary).get("stable_id", ""))).strip_edges()
	if value is Object:
		var object := value as Object
		if object.has_meta("actor_id"):
			return str(object.get_meta("actor_id")).strip_edges()
		if object.has_meta("actor_record_id"):
			return str(object.get_meta("actor_record_id")).strip_edges()
		var actor_id = object.get("actor_id")
		if actor_id != null:
			return str(actor_id).strip_edges()
		var stable_id = object.get("stable_id")
		if stable_id != null:
			return str(stable_id).strip_edges()
	return ""


func _population_record(actor_id: String) -> Dictionary:
	var bridge := _get_gecs_world()
	if bridge != null and bridge.has_method("get_population_record_core"):
		var core_record = bridge.call("get_population_record_core", actor_id)
		if core_record is Dictionary:
			return core_record as Dictionary
	if bridge != null and bridge.has_method("get_population_record"):
		var record = bridge.call("get_population_record", actor_id)
		if record is Dictionary:
			return record as Dictionary
	return {}


func _get_population_records() -> Dictionary:
	var bridge := _get_gecs_world()
	if bridge != null and bridge.has_method("get_population_records_core"):
		var core_records = bridge.call("get_population_records_core")
		if core_records is Dictionary:
			return core_records as Dictionary
	if bridge != null and bridge.has_method("get_population_records"):
		var records = bridge.call("get_population_records")
		if records is Dictionary:
			return records as Dictionary
	return {}


func _add_skill_xp(actor_id: String, skill_id: String, amount: float, records: Dictionary) -> void:
	if actor_id.is_empty() or amount <= 0.0:
		return
	var bridge := _get_gecs_world()
	if bridge == null or not bridge.has_method("upsert_population_record_core"):
		return
	var record: Dictionary = records.get(actor_id, {}) if records.get(actor_id, {}) is Dictionary else _population_record(actor_id)
	var patch := {"actor_id": actor_id}
	WORLD_ACTOR_RULES.add_skill_xp_to_patch(record, patch, skill_id, amount)
	if patch.size() > 1:
		bridge.call("upsert_population_record_core", patch)


func _actor_position(_actor_id: String, record: Dictionary) -> Vector3:
	var value = record.get("last_world_position", record.get("world_position", Vector3.ZERO))
	return value if value is Vector3 else Vector3.ZERO


func _perception_eye_position(actor_id: String, record: Dictionary) -> Vector3:
	return _actor_position(actor_id, record) + Vector3(0.0, DEFAULT_BODY_HEIGHT * 0.82, 0.0)


func _perception_forward_vector(_actor_id: String, record: Dictionary) -> Vector3:
	var yaw := float(record.get("world_facing_yaw", 0.0))
	return Vector3(sin(yaw), 0.0, cos(yaw)).normalized()


func _stealth_sample_positions(actor_id: String, record: Dictionary) -> Array[Vector3]:
	var position := _actor_position(actor_id, record)
	var height := DEFAULT_BODY_HEIGHT
	var side_offset := maxf(0.18, DEFAULT_BODY_RADIUS * 0.62)
	return [
		position + Vector3(0.0, height * 0.32, 0.0),
		position + Vector3(0.0, height * 0.58, 0.0),
		position + Vector3(0.0, height * 0.84, 0.0),
		position + Vector3(side_offset, height * 0.58, 0.0),
		position + Vector3(-side_offset, height * 0.58, 0.0),
	]


func _stealth_light_sample_position(actor_id: String, record: Dictionary) -> Vector3:
	return _actor_position(actor_id, record) + Vector3(0.0, DEFAULT_BODY_HEIGHT * 0.55, 0.0)


func _stealth_indicator_position(actor_id: String, record: Dictionary) -> Vector3:
	return _actor_position(actor_id, record) + Vector3(0.0, DEFAULT_BODY_HEIGHT + 0.65, 0.0)


func _current_camera() -> Camera3D:
	var viewport := get_viewport()
	if viewport != null:
		var camera := viewport.get_camera_3d()
		if camera != null:
			return camera
	return null


func _pair_key(observer_id: String, subject_id: String) -> String:
	return "%s:%s" % [observer_id, subject_id]


func _smoothstep(edge0: float, edge1: float, value: float) -> float:
	var x := clampf((value - edge0) / maxf(edge1 - edge0, 0.0001), 0.0, 1.0)
	return x * x * (3.0 - 2.0 * x)


func _get_gecs_world() -> Node:
	if _gecs_world_cache != null and is_instance_valid(_gecs_world_cache):
		return _gecs_world_cache
	var parent_node := get_parent()
	if parent_node != null:
		var local := parent_node.get_node_or_null("GecsWorldController")
		if local != null:
			_gecs_world_cache = local
			return local
	_gecs_world_cache = get_tree().get_first_node_in_group("gecs_world_controller") if is_inside_tree() else null
	return _gecs_world_cache


func _world_time_controller() -> Node:
	if _world_time_controller_cache != null and is_instance_valid(_world_time_controller_cache):
		return _world_time_controller_cache
	var parent_node := get_parent()
	_world_time_controller_cache = parent_node.get_node_or_null("WorldTimeController") if parent_node != null else null
	return _world_time_controller_cache


func _day_night_lighting_controller() -> Node:
	if _day_night_lighting_controller_cache != null and is_instance_valid(_day_night_lighting_controller_cache):
		return _day_night_lighting_controller_cache
	var parent_node := get_parent()
	_day_night_lighting_controller_cache = parent_node.get_node_or_null("DayNightLightingController") if parent_node != null else null
	return _day_night_lighting_controller_cache
