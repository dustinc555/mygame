...
static var node_write_enabled := not OS.get_cmdline_args().has("--combat-targeting-flip-off")
...
func query() -> QueryBuilder:
	return q.with_all([C_NODE, C_IDENTITY, C_SPATIAL, C_VITALS, C_FACTION, C_CONFIG, C_STATE]).iterate(
		[C_NODE, C_IDENTITY, C_SPATIAL, C_VITALS, C_FACTION, C_CONFIG, C_STATE])
...
func process(entities: Array, components: Array, _delta: float) -> void:
	...
	for i in range(count):
		...
		var radius: float = maxf(cfg_i.aggro_scan_radius, attack_range)
		...
		# Stickiness / hysteresis (mirrors COMBAT_COORDINATOR): keep the current target unless a
		# candidate is closer by more than attack_range * (RETARGET_MARGIN + STICKINESS) ≈ 2.4.
		var final_j := best_j
		var cur_id: int = state_i.current_target_id
		if cur_id != 0 and index_by_id.has(cur_id):
			var ci: int = index_by_id[cur_id]
			var vit_c = vitals[ci]
			var cfg_c = configs[ci]
			if vit_c != null and cfg_c != null and vit_c.life_state == alive_value and not cfg_c.protected_from_combat \
					and _is_hostile(fac_i, factions[ci], grudges_i, cur_id, grudge_actor_ids_i, actor_ids[ci], hostile_relation_pairs):
				var pos_c: Vector3 = spatials[ci].world_position
				if absf(pos_c.y - pos_i.y) <= vtol:
					var cdx := pos_c.x - pos_i.x
					var cdz := pos_c.z - pos_i.z
					var cur_d2 := cdx * cdx + cdz * cdz
					if best_j < 0:
						final_j = ci
					else:
						var keep_margin := attack_range * 2.4
						# keep current unless the best candidate is closer by more than keep_margin
						if sqrt(best_d2) >= sqrt(cur_d2) - keep_margin:
							final_j = ci
		if final_j >= 0:
			state_i.system_target_id = instance_ids[final_j]
			state_i.system_target_actor_id = str(identities[final_j].actor_id) if identities[final_j] != null else ""
...
func _write_node_target(actor, target_id: int, process_frame: int) -> void:
	if not node_write_enabled or actor == null or not is_instance_valid(actor):
		return
