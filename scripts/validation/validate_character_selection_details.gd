...
func _run_validation(scene: Node) -> void:
	...
	_validate_player_selection(selection_controller, details_panel, projection_controller, "player.mira", "Mira", "res://resources/items/steel_sword.tres", "res://resources/items/ranger_jerkin.tres")
	...
	_validate_player_selection(selection_controller, details_panel, projection_controller, "player.tomas", "Tomas", "res://resources/items/iron_axe.tres", "res://resources/items/peasant_tunic.tres")
...
func _validate_player_selection(selection_controller: Node, details_panel: Node, projection_controller: Node, actor_id: String, expected_name: String, expected_weapon: String, expected_chest: String) -> void:
	...
	_expect(not bool(panel_state.get("info_rows_visible", true)), "%s normal humanoid details hide world-target info rows" % expected_name)
	...
	_expect(not _joined_details_text(panel_state).contains("Damage"), "%s details panel does not show fake Damage bar" % expected_name)
