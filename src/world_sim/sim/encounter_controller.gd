...
const DEMAND_OBJECTIVE_PATH := "res://resources/world_sim/encounters/demand_tribute.tres"
...
func initialize(_target_root: Node, _target_hud: CanvasLayer = null) -> void:
	pass
...
func _get_settlement_controller() -> Node:
	...
	if parent_node != null:
		var local := parent_node.get_node_or_null("SettlementController")
		if local != null:
			return local
	return null
...
func _get_faction_controller() -> Node:
	var parent_node := get_parent()
	if parent_node != null:
		var local := parent_node.get_node_or_null("FactionController")
		if local != null:
			return local
	return get_tree().get_first_node_in_group("faction_controller") if get_tree() != null else null


func _get_faction_world_sim() -> Node:
	var parent_node := get_parent()
	if parent_node != null:
		var local := parent_node.get_node_or_null("FactionWorldSimController")
		if local != null:
			return local
	return get_tree().get_first_node_in_group("faction_world_sim_controller") if get_tree() != null else null


func _get_world_sim_squad_controller() -> Node:
	var parent_node := get_parent()
	if parent_node != null:
		var local := parent_node.get_node_or_null("WorldSimSquadController")
		if local != null:
			return local
	return get_tree().get_first_node_in_group("world_sim_squad_controller") if get_tree() != null else null
...
func _get_gecs_world() -> Node:
	...
	if parent_node != null:
		...
		if local != null:
			return local
	return get_tree().get_first_node_in_group("gecs_world_controller") if get_tree() != null else null
