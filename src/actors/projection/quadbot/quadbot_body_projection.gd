...
const STABLE_PHYSICAL_BONE_SCRIPT = preload("res://src/actors/projection/stable_physical_bone.gd")
...
func get_resolved_body_archetype() -> Resource:
	if actor != null and actor.get("body_archetype") != null:
		return actor.get("body_archetype") as Resource
	return null
...
func process_downed_visuals(delta: float) -> bool:
	if actor != null and bool(actor.get("_is_getting_up")):
		actor.set("_get_up_animation_remaining", maxf(0.0, float(actor.get("_get_up_animation_remaining")) - delta))
		return float(actor.get("_get_up_animation_remaining")) <= 0.0
	return false
...
func begin_get_up_visuals() -> void:
	if actor == null:
		return
	actor.set("_is_getting_up", true)
	actor.set("_get_up_animation_name", "")
	actor.set("_get_up_animation_total", 0.0)
	actor.set("_get_up_animation_remaining", 0.0)
	actor.call_deferred("_finish_get_up")


func cancel_get_up_visuals() -> void:
	if actor == null or not bool(actor.get("_is_getting_up")):
		return
	actor.set("_is_getting_up", false)
	actor.set("_get_up_animation_name", "")
	actor.set("_get_up_animation_remaining", 0.0)
	actor.set("_get_up_animation_total", 0.0)
...
func get_ragdoll_anchor_position() -> Variant:
	...
	return actor.global_position if actor is Node3D else null
...
func _get_quadbot_visual_scene() -> PackedScene:
	if actor == null:
		return null
	var archetype = actor.get("body_archetype")
	if archetype != null and archetype.get("visual_scene") != null:
		return archetype.get("visual_scene") as PackedScene
	if actor.has_method("get_quadbot_visual_scene"):
		return actor.call("get_quadbot_visual_scene") as PackedScene
	return null
...
func _start_quadbot_skeleton_ragdoll() -> bool:
	...
	_ragdoll_simulator.physical_bones_add_collision_exception(actor.get_rid())
...
func _stop_quadbot_skeleton_ragdoll(reset_pose: bool) -> void:
	if _ragdoll_simulator != null and is_instance_valid(_ragdoll_simulator):
		...
		if actor != null:
			_ragdoll_simulator.physical_bones_remove_collision_exception(actor.get_rid())
		_ragdoll_simulator.active = false
