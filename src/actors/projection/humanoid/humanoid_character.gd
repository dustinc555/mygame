...
const WORLD_TEXT_NOTICE_SCENE = preload("res://src/world/projection/effects/world_text_notice.tscn")
const COMBAT_ANIMATION_SET_SCRIPT = preload("res://resources/characters/combat_animation_set.gd")
const COMBAT_ATTACK_ANIMATION_SCRIPT = preload("res://resources/characters/combat_attack_animation.gd")
const HUMANOID_RAGDOLL_PROFILE_SCRIPT = preload("res://resources/characters/humanoid_ragdoll_profile.gd")
const STABLE_PHYSICAL_BONE_SCRIPT = preload("res://src/actors/projection/stable_physical_bone.gd")
const NEEDS_CAPABILITY_SCRIPT = preload("res://src/actors/bridge/capabilities/needs_capability.gd")
...
const HUMANOID_GRIP_SOCKET_MARKER_SCRIPT = preload("res://src/actors/projection/humanoid/humanoid_grip_socket_marker.gd")
...
func _process_ai(delta: float) -> void:
	...
	if _current_attack_target != null and not _is_valid_active_combat_target(_current_attack_target):
		stop_attack_assignment()
	# Fast path: the targeting system owns target choice (0.5s cadence + stickiness). If we are already
	# engaging exactly its current pick there is nothing for the node to re-decide this frame — the
