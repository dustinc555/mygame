...
const JAIL_FUNCTION = preload("res://resources/world_sim/facility_functions/jail.tres")
const DEFAULT_BUILDING_SCENE = preload("res://src/world/projection/buildings/settlement_keep_building.tscn")
const FACTION_HUMANOID_SCRIPT = preload("res://src/actors/projection/humanoid/faction_humanoid.gd")
const SETTLEMENT_GUARD_POST_SCRIPT = preload("res://src/settlements/bridge/venues/settlement_guard_post.gd")
const JAIL_CELL_SCRIPT = preload("res://src/settlements/bridge/jail_cell.gd")
const JAIL_CELL_SCENE = preload("res://src/settlements/bridge/jail_cell.tscn")
const PRISONER_LOCKER_SCENE = preload("res://src/world/projection/containers/prisoner_locker_container.tscn")
...
func _get_conversation_controller() -> Node:
	...
	if current_scene != null:
		var bootstrap := current_scene.get_node_or_null("GameBootstrap")
		if bootstrap != null:
			return bootstrap.get_node_or_null("ConversationController")
	return null
...
func _now_minute() -> int:
	...
	var bootstrap := root.get_node_or_null("GameBootstrap") if root != null else null
	var world_time := bootstrap.get_node_or_null("WorldTimeController") if bootstrap != null else null
	return int(world_time.call("get_absolute_minute")) if world_time != null and world_time.has_method("get_absolute_minute") else 0
