...
## Record-layer default stance for settlement roles: enforcers initiate and
## pursue; everyone else defends themselves without starting anything.
static func combat_stance_for_role(role_id: String) -> int:
	return CombatStance.AGGRESSIVE if role_id in ["guard", "warden"] else CombatStance.DEFENSIVE
...
const COMBAT_GRUDGE_SECONDS := 12.0
# Aggressive actors chase a live grudge target this far beyond their normal
# aggro scan radius before the grudge decays and they give up.
const COMBAT_PURSUIT_RANGE := 24.0
const SQUAD_ASSIST_RANGE := 100.0
