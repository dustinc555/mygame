...
func _ready() -> void:
	add_to_group("world_loader")
	# Compatibility group for existing controllers while WorldDefinition becomes canonical.
...
func get_world_id() -> String:
	return str(world_definition.call("get_id")) if world_definition != null and world_definition.has_method("get_id") else ""
...
func _sync_registry_arrays() -> void:
	...
	if world_definition == null:
		return
	if world_definition.has_method("get_all_faction_definitions"):
		faction_definitions = world_definition.call("get_all_faction_definitions")
	else:
		faction_definitions = _resource_array(world_definition.get("faction_definitions"))
	if world_definition.has_method("get_settlement_definitions"):
		settlement_definitions = world_definition.call("get_settlement_definitions")
	else:
		settlement_definitions = _settlement_definitions_from_placements()
	squad_templates = _resource_array(world_definition.get("squad_templates"))
	relation_definitions = _resource_array(world_definition.get("starting_relations"))
