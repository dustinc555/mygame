...
class_name HumanoidGripSocketMarker

const VISUAL_NAME := "__HumanoidGripSocketVisual"
...
const ORIGIN_COLOR := Color(0.45, 1.0, 0.95, 1.0)
...
@export var axis_length := 0.2:
	set(value):
		axis_length = value
		_rebuild_visual()
@export var center_radius := 0.022:
	set(value):
		center_radius = value
		_rebuild_visual()
...
func _rebuild_visual() -> void:
	_clear_visual()
	if not Engine.is_editor_hint() and not show_runtime_visual:
		return
