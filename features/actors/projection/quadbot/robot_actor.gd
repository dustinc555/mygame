extends "res://features/actors/bridge/world_actor.gd"

class_name RobotActor

const ROBOT_OIL = preload("res://features/world/resources/bleed_fluids/robot_oil.tres")
## Robots drive the combat coordinator directly (release_character on death, etc.); humanoids go
## through capabilities. So this preload lives on the robot subclass that actually uses it, NOT on
## the base WorldActor — keeping WorldActor out of a cycle with combat_coordinator. Inherited by quadbot.
...
func get_death_profile() -> int:
	return CGameActorVitals.DeathProfile.ROBOT
...
func _get_bleed_splotch_controller() -> Node:
	...
	if _bleed_splotch_controller_lookup_cooldown > 0.0:
		return null
	# Projection effect sink discovered by group (supports test doubles + multiple
	# sinks), not singleton controller wiring -- so this stays a group lookup.
	var tree := get_tree()
	var controllers := tree.get_nodes_in_group("bleed_splotch_controller") if tree != null else []
