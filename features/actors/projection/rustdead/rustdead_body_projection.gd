...
func apply_cinder_burned_visuals() -> void:
	...
	var body_mesh := _actor.get_node_or_null("BodyMesh")
	if body_mesh != null:
...
func clear_cinder_burned_visuals() -> void:
	...
	var body_mesh := _actor.get_node_or_null("BodyMesh") if actor != null else null
	if body_mesh != null:
...
func _update_cinder_burn_effect_position() -> void:
	if _cinder_burn_effect == null or not is_instance_valid(_cinder_burn_effect):
		return
	_cinder_burn_effect.global_position = _actor.get_follow_anchor_position() + Vector3(0.0, 0.35, 0.0)
