...
const DEFAULT_PROFILE_PATH := "res://features/ai/resources/utility_profiles/default_humanoid.tres"
...
const AI_UTILITY_TARGET_SELECTOR_SCRIPT := preload("res://features/ai/sim/ai_utility_target_selector.gd")
const AI_UTILITY_CONTEXT_SCRIPT := preload("res://features/ai/sim/ai_utility_context.gd")
const AI_UTILITY_JOB_FACTORY_SCRIPT := preload("res://features/ai/sim/ai_utility_job_factory.gd")
...
func _apply_order_facts(context: AiUtilityContext, actor: Node) -> void:
	if actor is WorldActor:
		...
		return
	var actor_interaction := (actor as WorldActor).get_interaction() if actor is WorldActor else null
	var order_was_player_issued := actor_interaction.order_was_player_issued if actor_interaction != null else false
	var current_order_type := actor_interaction.current_order_type if actor_interaction != null else -1
