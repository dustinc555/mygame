		...
		for cell_x in range(center_cell.x - cell_radius, center_cell.x + cell_radius + 1):
			for cell_z in range(center_cell.y - cell_radius, center_cell.y + cell_radius + 1):
				...
				for j_value in bucket:
					...
					if d2 > radius_sq:
						continue
					# Beyond normal scan range only grudge targets qualify (pursuit).
					if d2 > base_radius_sq and not (grudges_i.has(instance_ids[j]) or grudge_actor_ids_i.has(actor_ids[j])):
						continue
...
func _engagement_lock_index(i: int, slots: Array, index_by_actor_id: Dictionary, vitals: Array, configs: Array, factions: Array, instance_ids: PackedInt64Array, actor_ids: PackedStringArray, alive_value: int, hostile_relation_pairs: Dictionary, fac_i, grudges_i: PackedInt64Array, grudge_actor_ids_i: PackedStringArray, require_grudge := false) -> int:
	...
	if vit_j == null or cfg_j == null or vit_j.life_state != alive_value or cfg_j.protected_from_combat:
		return -1
	# The lock also releases for defensive fighters when the opponent starts
