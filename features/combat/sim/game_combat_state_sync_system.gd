...
# Typed node -> CGameCombatLoadout author. The actor owns the source-stat resolution;
# GameCombatScoreSystem (registered after this) turns the loadout into packed scores.
func _author_loadout(entity, actor: Node) -> void:
	if entity == null:
		return
	var world_actor := actor as WorldActor
	if world_actor == null:
		return
	var loadout = entity.get_component(C_LOADOUT)
	if loadout != null:
		world_actor.write_combat_loadout(loadout)


func _sync_config(component, actor: Node) -> void:
	if component == null:
		return
	var order_actor := actor as WorldActor
	component.player_order_active = order_actor.has_active_player_order() if order_actor != null else false
	...
	# blunt/cut damage, hit/dodge/block/crit scores, toughness, has_shield and weapon_skill_id are
	# now derived by GameCombatScoreSystem from CGameCombatLoadout (authored in _author_loadout).
	# No reflection into dead combat getters here.
...
func _sync_state(component, actor: Node) -> void:
	...
	var grudge_actor := actor as WorldActor
	var grudges = grudge_actor.get_active_grudges() if grudge_actor != null else actor.get("_personal_hostile_ids")
