...
const ACTOR_CONDITION_EVALUATOR_SCRIPT = preload("res://features/settlements/sim/actor_condition_evaluator.gd")
...
func tick_worker_job_from_ai(worker: WorldActor, delta: float, _ai_job = null) -> Dictionary:
	...
	# this through AI-job priority; the legacy job system predates the order model.)
	if worker != null:
		var order_type := worker.get_current_order_type()
		if worker.get_carried_character() != null 				or (order_type != InteractionCapability.ORDER_TYPE_NONE and order_type != InteractionCapability.ORDER_TYPE_MOVE):
			return {"active": true, "ended": false, "blocker": "Worker busy with a priority order"}
